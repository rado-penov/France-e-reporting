 ...
 *   custscript_pret_api_function_key_vb  Free-form Text — Value sent as the X-Function-Key header
...
(record, file, https, runtime, log) => {
    ...
    function afterSubmit(context) {
        ...
        try {
            ...
            const apiKey     = script.getParameter({ name: 'custscript_pret_api_function_key_vb' });
            ...
            log.audit('UBL API PARAMS', `Bill: ${tranId} | url: ${apiUrl || '(empty)'} | functionKey set: ${!!apiKey} | docType: ${apiDocType || '(empty)'}`);
            ...
            if (apiUrl && apiKey && apiDocType) {
                ...
                log.error('UBL API SKIPPED', `Bill: ${tranId} | Missing parameters — url: ${!!apiUrl} | functionKey: ${!!apiKey} | docType: ${!!apiDocType}`);
            }
