 ...
 *
 * Builds a B2B Bills (Vendor Bill) Payments Report XML from saved search 'customsearch_pret_france_bills_b2b',
 ...
 *   InvoiceID     — Vendor Bill Reference No. (grouped)         → PaymentsReport/Invoice/InvoiceID
 *   IssueDate     — Vendor Bill Date (grouped)                → PaymentsReport/Invoice/IssueDate
 *   PaymentDate   — Bill Payment Date (grouped)               → PaymentsReport/Invoice/Payment/Date
 ...
 *
 * Each search result row is one (vendor bill, payment date, tax rate) combination; rows are grouped
 * into one <Invoice> element per (bill, payment date) with one <SubTotals> per tax rate.
 *
...
(search, file, https, runtime, format, encode, log) => {
    ...
    function execute(context) {
        try {
            ...
            if (!window) {
                log.audit('B2B BILLS REPORT SKIPPED', `Today (${fmtYYYYMMDD(today)}) is not the 1st, 11th or 21st — nothing to run`);
                return;
            }
            log.audit('B2B BILLS REPORT WINDOW', `Today: ${fmtYYYYMMDD(today)} | Window: ${fmtYYYYMMDD(window.start)} - ${fmtYYYYMMDD(window.end)}`);

            log.audit('B2B BILLS REPORT SEARCH START', `Search: ${SEARCH_ID} | Filter trandate within: ${toFilterDate(window.start)} - ${toFilterDate(window.end)}`);
            const payments = runSearch(window);
            log.audit('B2B BILLS REPORT SEARCH RESULTS', `Rows returned: ${payments.length}`);
            if (payments.length === 0) {
                log.audit('B2B BILLS REPORT SKIPPED', `No results for window ${fmtYYYYMMDD(window.start)} - ${fmtYYYYMMDD(window.end)}`);
                return;
            }
            payments.forEach((p, i) => log.debug('B2B BILLS REPORT ROW', `#${i + 1} | Invoice: ${p.invoiceId} | IssueDate: ${p.issueDate} | Date: ${p.date} | TaxPercent: ${p.taxPercent} | Currency: ${p.currencyCode} | Amount: ${p.amount}`));

            const rptId = `RPT-${today.getFullYear()}-${pad2(today.getDate())}${pad2(today.getMonth() + 1)}`;
            log.audit('B2B BILLS REPORT ID', `RPT ID: ${rptId}`);

            const invoices = groupPaymentsByInvoice(payments);
            log.debug('B2B BILLS REPORT GROUPED', `Rows: ${payments.length} | Invoice elements: ${invoices.length}`);

            const nowStamp = nowTimestamp();
            const xml = buildReportXml(rptId, invoices, nowStamp);
            log.debug('B2B BILLS REPORT XML BUILT', `Length: ${xml.length} chars`);

            const fileName = `${nowStamp.slice(2, 8)}_B2BBillsReport_RPT_${nowStamp.slice(-4)}.xml`;
            log.audit('B2B BILLS REPORT FILE SAVING', `File: ${fileName} | Folder: ${folderId}`);
            ...
            log.audit('B2B BILLS REPORT FILE SAVED', `File: ${fileName} | File ID: ${fileId}`);
            ...
            if (apiUrl && oauthConfigured && apiDocType) {
                try {
                    ...
                    if (response.code >= 200 && response.code < 300) {
                        log.audit('B2B BILLS REPORT API SENT', `Status: ${response.code}`);
                    } else {
                        log.error('B2B BILLS REPORT API FAILED', `Status: ${response.code} | Body: ${response.body}`);
                    }
                } catch (apiErr) {
                    log.error('B2B BILLS REPORT API FAILED', `Name: ${apiErr.name} | Message: ${apiErr.message} | Stack: ${apiErr.stack}`);
                }
            } else {
                log.error('B2B BILLS REPORT API SKIPPED', `Missing parameters — url: ${!!apiUrl} | oauth configured: ${oauthConfigured} | docType: ${!!apiDocType}`);
            }

            log.audit('B2B BILLS REPORT COMPLETE', `RPT ID: ${rptId} | File ID: ${fileId} | Invoices: ${invoices.length}`);

        } catch (e) {
            log.error('B2B BILLS REPORT FAILED', `${e.message}\n${e.stack}`);
        }
    }
    ...
    function runSearch(window) {
        ...
        log.debug('B2B BILLS REPORT SEARCH COLUMNS', `Labels found: ${Object.keys(labelMap).join(', ')}`);
        ...
        log.debug('B2B BILLS REPORT SEARCH PAGED', `Total pages: ${pagedData.pageRanges.length} | Total count reported: ${pagedData.count}`);
        for (let p = 0; p < pagedData.pageRanges.length; p++) {
            const page = pagedData.fetch({ index: p });
            log.debug('B2B BILLS REPORT PAGE FETCHED', `Page: ${p + 1}/${pagedData.pageRanges.length} | Rows: ${page.data.length}`);
    ...
    // of SubTotals — a bill/payment can split across multiple tax rates (same shape as the B2B
    // payments report, where one Payment carries e.g. a 20% and a 10% SubTotals).
